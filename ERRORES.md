...
- **E25 (UI) — Paneles de la barra superior se abrían fuera de cámara y enormes.** 🔧 ARREGLADO (Fase 0.3). Causa real: los paneles se agregaban a `get_tree().root` FUERA de un CanvasLayer → la cámara del mapa los arrastraba fuera de vista. Ahora todos cuelgan del `UILayer` (CanvasLayer) vía `_ui_host()`, anclados a la pantalla. Además la pantalla de Agentes se redujo de 1400×760 a 1120×680. Verificado headless: exit 0, 0 errores. (Confirmar visualmente.)
