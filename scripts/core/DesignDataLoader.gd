...
func load_modules() -> void:
	...
	Log.info("✅ Equipment modules loaded: " + str(modules.size()), "DesignDataLoader")


func load_templates() -> void:
	templates = _load_json_objects_from_dir(TEMPLATES_DIR, UnitTemplate.from_dict)
	Log.info("✅ Unit templates loaded: " + str(templates.size()), "DesignDataLoader")


func load_production_rules() -> void:
	production_rules = _load_json_dict(RULES_PATH)
	if production_rules.is_empty():
		Log.warn("Production line rules missing or invalid: " + RULES_PATH)
	else:
		Log.info("✅ Production line rules loaded", "DesignDataLoader")
...
func load_sustainment_equipment() -> void:
	...
	if dir == null:
		Log.warn("DesignDataLoader: sustainment equipment folder missing: ", SUSTAINMENT_DIR)
		return
	dir.list_dir_begin()
	...
	dir.list_dir_end()
	Log.info("✅ Sustainment equipment loaded: " + str(sustainment_templates.size()), "DesignDataLoader")
...
func _load_json_objects_from_dir_recursive(
	...
	if dir == null:
		Log.warn("Could not open directory: " + dir_path)
		return
	dir.list_dir_begin()
	var file_name := dir.get_next()
	while file_name != "":
...
func _load_json_dict(path: String) -> Dictionary:
	if not FileAccess.file_exists(path):
		Log.warn("Missing JSON file: " + path)
		return {}
	var file := FileAccess.open(path, FileAccess.READ)
	if not file:
		Log.warn("Could not open JSON file: " + path)
		return {}
	var json := JSON.new()
	var parse_result := json.parse(file.get_as_text())
	file.close()
	if parse_result != OK or typeof(json.data) != TYPE_DICTIONARY:
		Log.warn("Failed to parse JSON: " + path)
		return {}
	return json.data
